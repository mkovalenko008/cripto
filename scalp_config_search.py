"""
Поиск конфигурации скальп-идеи ("пара тиков" на спокойных монетах) на
5-минутных свечах. Дисциплина та же, что у trend_config_search.py и
bb_config_search_v2.py: сетка крутится СТРОГО по train, победитель выбирается
только по train, test прогоняется один раз без подгонки.

Контроль — target_pct=0.15% — сознательно взят МЕНЬШЕ комиссии в оба конца
(0.2%). Если он вдруг не худший на train, это первый сигнал, что с бэктестом
что-то не так (утечка данных, неправильный учёт комиссии и т.п.), а не что
"пара тиков в буквальном смысле" внезапно работает — экономика сделки просто
не может дать плюс, если цель меньше суммарной комиссии.

MIN_TRADES/MIN_COVERAGE — те же по смыслу пороги надёжности, что в
bb_config_search_v2.py, но у скальпа сделок на порядок больше на тех же
днях истории (мельче таймфрейм -> чаще сигналы), поэтому пороги строже.
"""
...
TARGET_PCT_VARIANTS = [0.15, 0.25, 0.4, 0.6, 0.9]
STOP_PCT_VARIANTS = [0.4, 0.6, 0.9]
MAX_BARS_VARIANTS = [12, 24]
CONTROL = (20, 2.0, 0.15, 0.6, 12)   # target=0.15% < комиссии за круг — заведомо убыточный ориентир
...
def main():
    ...
    if not data:
        print("Нет данных в data/*_5min.json — сначала fetch_scalp_basket.py")
        return
    print(f"Спокойных монет с данными: {len(data)} из {len(symbols)}, таймфрейм 5min\n")
    ...
    lines = ["=" * 96,
             "SCALP — поиск по TRAIN (5min, только LONG на споте, комиссия 0.2% за круг)",
             "=" * 96]
    ...
    lines.append(f"... показаны топ-25 из {len(results)} конфигураций (полные — не в отчёте, при "
                 f"необходимости перезапусти main() и распечатай `results`)")
    ...
    lines.append("и удерживает широту/медиану, сопоставимые с train — иначе это подгонка под train, не edge.")

    text = "\n".join(lines)
    print("\n" + text)
    with open(os.path.join(os.path.dirname(__file__), "scalp_config_search_report.txt"), "w") as f:
        f.write(text + "\n")
